...
import { activeGame, activeGameSettings, lobbySettings, lobbyMembers } from "../models/stores";
...
export function beginGames() {
  ...
  if (length === 2) {
    ...
    activeGameSettings.set({ circlePlayerUid: playerUids[0], crossPlayerUid: playerUids[1], gameSettings: get(lobbySettings)})
  } else if (length > 2) {
